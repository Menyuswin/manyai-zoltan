...
Deployment is GitHub Pages serving directly from the `main` branch root — a push to `main` is a push to production. Live URL: `https://manyai.hu/` (custom domain via the `CNAME` file; the old `https://menyuswin.github.io/manyai-zoltan/` 301-redirects there).
...
- All absolute URLs in the head (canonical, `og:url`, `og:image`, JSON-LD `url`/`@id`/`image`) and in `sitemap.xml` must use `https://manyai.hu/`, never the old github.io path: that path now 301-redirects, and messaging apps don't follow redirects for preview data — after the domain switch, pasting manyai.hu produced a bare, image-less preview until these were updated (2026-09-26).
- `og:image` must be an **absolute** URL (`https://manyai.hu/og-image-v2.jpg`) — relative paths break previews in Signal/WhatsApp/LinkedIn. Messaging apps cache previews aggressively; after changing the image, change its filename (and the tags) to bust caches.
- `og-image-v2.jpg` (1200×630; renamed from `og-image.jpg` to bust preview caches after the domain switch) was composed from a background-removed portrait (rembg, `isnet-general-use` model) on a dark navy gradient with the site's gold accent, rendered from HTML with Playwright. The face sits near the horizontal center on purpose — WhatsApp and some clients center-crop to a square thumbnail. The site's Google Fonts are not reachable from the sandbox; for faithful renders load them locally from npm `@fontsource/{newsreader,public-sans,ibm-plex-mono}`.
...
- There is currently **no `robots.txt`**. It used to be pointless (the site lived under `/manyai-zoltan/`, and crawlers only read it at the domain root); since the site is now served at the root of `manyai.hu`, one here *would* take effect if ever needed. `sitemap.xml` is submitted directly in Google Search Console instead; bump its `<lastmod>` dates when page content changes meaningfully.
...
**Topbar nav renders as buttons, not text links** (`style.css`, `.topbar-nav a`): solid accent-colored fill, same visual weight as the `.topbar-cta` "Kapcsolat" button. The current page is marked with `aria-current="page"` on its own nav link, which flips it to an inverse/outline treatment via `.topbar-nav a[aria-current="page"]` — this is the only way "you are here" is indicated, so don't reintroduce a second "Kapcsolat" entry or drop `aria-current` when touching nav markup. At ≤720px the nav is a 2×2 grid (it used to be a single horizontally-scrolling row, which silently pushed "Produktumok" off-screen on phones with no scroll hint — don't go back to that). Because the sticky topbar is ~130px tall on phones, `section[id]` carries a `scroll-margin-top` (24px desktop / 96px mobile) so anchor jumps don't land the section heading under the bar; retune it if the topbar height changes.
